...
- Python CultNet RUDP transport:
  ...
  advertised through RUDP transport profiles. `CultNetRudpReconnectLoop`
  consumes that controller for caller-owned RUDP transport factories, matching
  the TypeScript/Kotlin reconnect-loop shape without making the socket factory
  a hidden global. The TypeScript interop harness proves bidirectional Python/TypeScript RUDP socket exchange,
...
- Python CultMesh discovery client:
  ...
  metadata on health results including document types, message versions, and
  mutation contracts, run immediate scheduled health probes on start, and
  upsert results into local catalogs with the same count-returning ergonomics
  as the C# discovery clients.
- Python CultMesh facade:
  ...
  peer-card contact hints. Peer catalogs expose authorized-peer lookup, and the
  branded facade can open a RUDP client from the first peer authorized by the
  authority lease catalog for a Verse role, keeping endpoint contact separate
  from trust. The durable shard-log node option attaches a file-backed
  authoritative log store beside the cache file by default. Schema, shard, and
  RUDP factories delegate to the existing `cultnet_py` owners, so the branded
  entrypoint does not create a second source of discovery or transport truth.
  The `cultmesh-py-daemon` /

...
const bytes = new ArrayBuffer(152); const view = new DataView(bytes); let offset = 0;
...
u32(1); u64(0x1234n); u32(2); u32(17); i32(0); f64(0); f64(0); f64(1);
i32(0); i32(0); i32(0); i32(2); i32(0); f32(10); f32(0.5); i32(4); u8(1);
for (const value of [1, 0, 0]) f32(value); f32(8.4); f32(0.25);
...
u8(0); u8(0); u8(0); assert.equal(offset, bytes.byteLength);
const tile = decodePlanetarySurfaceTile(bytes);
assert.equal(tile.header.fieldVersion, 0x1234n); assert.equal(tile.header.layerId, 17);
assert.equal(tile.storageSize, 2); assert.equal(tile.samples.length, 4);
assert.equal(tile.samples[0]?.radialDisplacement, 0.25); assert.equal(tile.samples[1], null);
assert.throws(() => decodePlanetarySurfaceTile(bytes.slice(0, -1)), /Truncated/);

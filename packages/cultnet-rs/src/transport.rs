    ...
    TStream: Read,
{
    pub fn receive(&mut self) -> Result<CultNetTransportFrame> {
        ...
        // Refuse before allocating. The length is the peer's claim, so honouring
        // it without checking the advertised maximum lets one header reserve as
        // much memory as the peer cares to name.

...
#ifndef GAMECULT_GEOMETRY_ADVANCED_EROSION_FILTER_HLSL
#define GAMECULT_GEOMETRY_ADVANCED_EROSION_FILTER_HLSL

struct GameCultGeometryAdvancedErosionParameters
{
...
struct GameCultGeometryAdvancedErosionResult
{
    float3 delta; float magnitude; float ridge_map; float fade_target;
};

struct GameCultGeometryErosionBandSelection
{
    int active_octaves; float final_octave_weight; float finest_included_wavelength; float unresolved_height_bound;
};

GameCultGeometryErosionBandSelection gamecult_geometry_select_erosion_bands(float base_wavelength, float sample_spacing, int maximum_octaves, float lacunarity, float base_amplitude, float gain, float transition_ratio)
{
    ...
    GameCultGeometryErosionBandSelection result; result.active_octaves=active; result.final_octave_weight=active>0?final_weight:0.0; result.finest_included_wavelength=finest; result.unresolved_height_bound=omitted; return result;
}

float gamecult_geometry_erosion_hash_unit(int2 cell, uint salt)
{
...
float2 gamecult_geometry_erosion_hash2(float2 input)
{
    int2 cell = (int2)input;
    return float2(gamecult_geometry_erosion_hash_unit(cell, 0x68bc21ebu), gamecult_geometry_erosion_hash_unit(cell, 0x02e5be93u)) * 2.0 - 1.0;
}

float4 gamecult_geometry_phacelle_noise(float2 position, float2 normal_direction, float frequency, float offset_cycles, float normalization)
{
    ...
    [unroll] for (int y = -1; y <= 2; y++)
    {
        ...
        float2 random_offset = gamecult_geometry_erosion_hash2(grid_point) * 0.5;
...
float gamecult_geometry_erosion_pow_inverse(float value, float power) { return 1.0 - pow(1.0 - saturate(value), power); }
float gamecult_geometry_erosion_ease_out(float value) { float inverse = 1.0 - saturate(value); return 1.0 - inverse * inverse; }
float gamecult_geometry_erosion_smooth_start(float value, float smoothing)
{
    return smoothing <= 1.0e-10 || value >= smoothing ? value - 0.5 * smoothing : 0.5 * value * value / smoothing;
}
float2 gamecult_geometry_erosion_safe_normalize(float2 value)
{
    float value_length = length(value);
    return abs(value_length) > 1.0e-10 ? value / value_length : value;
}

GameCultGeometryAdvancedErosionResult gamecult_geometry_advanced_erosion_filter_banded(float2 position, float3 base_height_and_slope, float fade_target, GameCultGeometryAdvancedErosionParameters p, GameCultGeometryErosionBandSelection band)
{
    ...
    float combined_mask = gamecult_geometry_erosion_ease_out(gamecult_geometry_erosion_smooth_start(slope_length * p.onset.x, input_rounding * p.onset.x));
    float ridge_mask = gamecult_geometry_erosion_ease_out(slope_length * p.onset.z);
    ...
    {
        float4 phacelle = gamecult_geometry_phacelle_noise(position * frequency, gamecult_geometry_erosion_safe_normalize(gully_slope), p.cell_scale, 0.25, p.normalization);
        ...
        float new_mask = gamecult_geometry_erosion_ease_out(gamecult_geometry_erosion_smooth_start(sloping * p.onset.y, octave_rounding * p.onset.y));
        combined_mask = gamecult_geometry_erosion_pow_inverse(combined_mask, p.detail) * new_mask;
        ridge_fade = lerp(ridge_fade, lerp(ridge_fade, gullies.x, ridge_mask), octave_weight);
        ridge_mask *= lerp(1.0, gamecult_geometry_erosion_ease_out(sloping * p.onset.w), octave_weight);
        strength *= p.gain;
        frequency *= p.lacunarity;
        rounding_multiplier *= p.rounding.w;
    }

    GameCultGeometryAdvancedErosionResult result;
...
GameCultGeometryAdvancedErosionResult gamecult_geometry_advanced_erosion_filter(float2 position, float3 base_height_and_slope, float fade_target, GameCultGeometryAdvancedErosionParameters p)
{
    GameCultGeometryErosionBandSelection band; band.active_octaves=p.octaves; band.final_octave_weight=1.0; band.finest_included_wavelength=0.0; band.unresolved_height_bound=0.0;
    return gamecult_geometry_advanced_erosion_filter_banded(position, base_height_and_slope, fade_target, p, band);
}

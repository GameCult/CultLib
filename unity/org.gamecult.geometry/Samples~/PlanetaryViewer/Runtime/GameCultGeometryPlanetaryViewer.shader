Shader "GameCult.Geometry/Planetary Field Viewer"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            GameCultGeometryPlanetarySurfaceSample viewer_sample(float3 direction)
            {
                ...
                return gamecult_geometry_planetary_field_sample(definition, direction, base_sample, _SampleFootprint);
            }

            Varyings vert(Attributes input)
            {
                float3 direction = normalize(input.vertex.xyz);
                GameCultGeometryPlanetarySurfaceSample sample = viewer_sample(direction);

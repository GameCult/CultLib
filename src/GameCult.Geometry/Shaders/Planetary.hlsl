#ifndef GAMECULT_GEOMETRY_PLANETARY_HLSL
#define GAMECULT_GEOMETRY_PLANETARY_HLSL

static const float GAMECULT_GEOMETRY_PLANETARY_QUARTER_PI = 0.78539816339744830962;

float3 gamecult_geometry_planetary_face_direction(int face, float2 coordinate)
{
    float2 tangent = tan(coordinate * GAMECULT_GEOMETRY_PLANETARY_QUARTER_PI);
...
float2 gamecult_geometry_planetary_face_coordinate(float3 direction, out int face)
{
    ...
    return atan(cube_uv) / GAMECULT_GEOMETRY_PLANETARY_QUARTER_PI;
}

bool gamecult_geometry_planetary_page_local(float3 direction, float4 tile_address, float active, out float2 local)
{
    int face;
    float2 uv = gamecult_geometry_planetary_face_coordinate(direction, face);
    local = 0.0;
...
float3 gamecult_geometry_planetary_surface_normal(float3 direction, float3 world_distance_tangent_gradient)
{
...
float2 gamecult_geometry_planetary_equirectangular_forward(float3 direction)
{
    direction = cultmath_normalize(direction);
    return float2(atan2(direction.y, direction.x) / CULTMATH_PI, asin(clamp(direction.z, -1.0, 1.0)) / CULTMATH_HALF_PI);
}

float3 gamecult_geometry_planetary_equirectangular_inverse(float2 coordinate)
{
...
static const float GAMECULT_GEOMETRY_EQUAL_EARTH_A1 = 1.340264;
static const float GAMECULT_GEOMETRY_EQUAL_EARTH_A2 = -0.081106;
static const float GAMECULT_GEOMETRY_EQUAL_EARTH_A3 = 0.000893;
static const float GAMECULT_GEOMETRY_EQUAL_EARTH_A4 = 0.003796;
static const float GAMECULT_GEOMETRY_EQUAL_EARTH_M = 0.8660254037844386;
static const float GAMECULT_GEOMETRY_EQUAL_EARTH_X_MAX = 2.706629;
static const float GAMECULT_GEOMETRY_EQUAL_EARTH_Y_MAX = 1.3173628;

float gamecult_geometry_equal_earth_y(float theta)
{
    float theta2 = theta * theta;
    float theta6 = theta2 * theta2 * theta2;
    return theta * (GAMECULT_GEOMETRY_EQUAL_EARTH_A1 + GAMECULT_GEOMETRY_EQUAL_EARTH_A2 * theta2 + theta6 * (GAMECULT_GEOMETRY_EQUAL_EARTH_A3 + GAMECULT_GEOMETRY_EQUAL_EARTH_A4 * theta2));
}

float2 gamecult_geometry_planetary_equal_earth_forward(float3 direction)
{
    direction = cultmath_normalize(direction);
    float longitude = atan2(direction.y, direction.x);
    float latitude = asin(clamp(direction.z, -1.0, 1.0));
    float theta = asin(GAMECULT_GEOMETRY_EQUAL_EARTH_M * sin(latitude));
    float theta2 = theta * theta;
    float theta6 = theta2 * theta2 * theta2;
    float denominator = GAMECULT_GEOMETRY_EQUAL_EARTH_M * (GAMECULT_GEOMETRY_EQUAL_EARTH_A1 + 3.0 * GAMECULT_GEOMETRY_EQUAL_EARTH_A2 * theta2 + theta6 * (7.0 * GAMECULT_GEOMETRY_EQUAL_EARTH_A3 + 9.0 * GAMECULT_GEOMETRY_EQUAL_EARTH_A4 * theta2));
    return float2(longitude * cos(theta) / denominator / GAMECULT_GEOMETRY_EQUAL_EARTH_X_MAX, gamecult_geometry_equal_earth_y(theta) / GAMECULT_GEOMETRY_EQUAL_EARTH_Y_MAX);
}

float3 gamecult_geometry_planetary_equal_earth_inverse(float2 coordinate)
{
    float target_y = coordinate.y * GAMECULT_GEOMETRY_EQUAL_EARTH_Y_MAX;
    float theta = target_y / GAMECULT_GEOMETRY_EQUAL_EARTH_A1;
    [unroll] for (int iteration = 0; iteration < 8; iteration++)
    {
        float theta2 = theta * theta;
        float theta6 = theta2 * theta2 * theta2;
        float derivative = GAMECULT_GEOMETRY_EQUAL_EARTH_A1 + 3.0 * GAMECULT_GEOMETRY_EQUAL_EARTH_A2 * theta2 + theta6 * (7.0 * GAMECULT_GEOMETRY_EQUAL_EARTH_A3 + 9.0 * GAMECULT_GEOMETRY_EQUAL_EARTH_A4 * theta2);
        theta -= (gamecult_geometry_equal_earth_y(theta) - target_y) / derivative;
    }
    float latitude = asin(clamp(sin(theta) / GAMECULT_GEOMETRY_EQUAL_EARTH_M, -1.0, 1.0));
    float theta2 = theta * theta;
    float theta6 = theta2 * theta2 * theta2;
    float denominator = GAMECULT_GEOMETRY_EQUAL_EARTH_M * (GAMECULT_GEOMETRY_EQUAL_EARTH_A1 + 3.0 * GAMECULT_GEOMETRY_EQUAL_EARTH_A2 * theta2 + theta6 * (7.0 * GAMECULT_GEOMETRY_EQUAL_EARTH_A3 + 9.0 * GAMECULT_GEOMETRY_EQUAL_EARTH_A4 * theta2));
    float longitude = coordinate.x * GAMECULT_GEOMETRY_EQUAL_EARTH_X_MAX * denominator / cos(theta);
    float cos_latitude = cos(latitude);
    return cultmath_normalize(float3(cos_latitude * cos(longitude), cos_latitude * sin(longitude), sin(latitude)));
}

float gamecult_geometry_planetary_wrap_longitude(float longitude)
{
...
float3 gamecult_geometry_planetary_lon_lat_direction(float longitude, float latitude)
{
    float cos_latitude = cos(latitude);
    return cultmath_normalize(float3(cos_latitude * cos(longitude), cos_latitude * sin(longitude), sin(latitude)));
}

float gamecult_geometry_planetary_asinh(float value) { return log(value + sqrt(value * value + 1.0)); }
float gamecult_geometry_planetary_sinh(float value) { return 0.5 * (exp(value) - exp(-value)); }

bool gamecult_geometry_planetary_azimuthal_forward(float3 direction, float2 center_lon_lat, int kind, out float2 coordinate)
{
    direction = cultmath_normalize(direction);
    float longitude = gamecult_geometry_planetary_wrap_longitude(atan2(direction.y, direction.x) - center_lon_lat.x);
    float latitude = asin(clamp(direction.z, -1.0, 1.0));
...
bool gamecult_geometry_planetary_azimuthal_inverse(float2 coordinate, float2 center_lon_lat, int kind, out float3 direction)
{
    ...
    if (rho < 1.0e-6) { direction = gamecult_geometry_planetary_lon_lat_direction(center_lon_lat.x, center_lon_lat.y); return true; }
    ...
    direction = gamecult_geometry_planetary_lon_lat_direction(gamecult_geometry_planetary_wrap_longitude(delta_longitude + center_lon_lat.x), latitude);
    return true;
}

bool gamecult_geometry_planetary_cube_atlas_forward(float3 direction, out float2 coordinate)
{
    int face;
    float2 face_coordinate = gamecult_geometry_planetary_face_coordinate(direction, face);
    int column = face % 3;
    int row = face / 3;
...
bool gamecult_geometry_planetary_cube_atlas_inverse(float2 coordinate, out float3 direction)
{
    ...
    direction = gamecult_geometry_planetary_face_direction(row * 3 + column, float2(u, v));
    return true;
}

bool gamecult_geometry_planetary_projection_forward(float3 direction, int kind, float3 center_lon_lat_scale, out float2 coordinate)
{
    float scale = max(center_lon_lat_scale.z, 1.0e-20);
    if (kind == 6)
    {
        bool valid = gamecult_geometry_planetary_cube_atlas_forward(direction, coordinate);
        coordinate /= scale; return valid;
    }
    if (kind >= 3)
    {
        bool valid = gamecult_geometry_planetary_azimuthal_forward(direction, center_lon_lat_scale.xy, kind, coordinate);
        coordinate /= scale; return valid;
    }
    direction = cultmath_normalize(direction);
    float longitude = gamecult_geometry_planetary_wrap_longitude(atan2(direction.y, direction.x) - center_lon_lat_scale.x);
    float latitude = asin(clamp(direction.z, -1.0, 1.0));
    ...
    else if (kind == 1)
    {
        ...
        coordinate = float2(longitude / CULTMATH_PI, gamecult_geometry_planetary_asinh(tan(latitude)) / CULTMATH_PI);
    }
    else
    {
        float3 centered = gamecult_geometry_planetary_lon_lat_direction(longitude, latitude);
        coordinate = gamecult_geometry_planetary_equal_earth_forward(centered);
    }
    coordinate /= scale;
    return true;
}

bool gamecult_geometry_planetary_projection_inverse(float2 coordinate, int kind, float3 center_lon_lat_scale, out float3 direction)
{
    coordinate *= max(center_lon_lat_scale.z, 1.0e-20);
    if (kind == 6) return gamecult_geometry_planetary_cube_atlas_inverse(coordinate, direction);
    if (kind >= 3) return gamecult_geometry_planetary_azimuthal_inverse(coordinate, center_lon_lat_scale.xy, kind, direction);
    ...
    else if (kind == 1) { longitude = coordinate.x * CULTMATH_PI; latitude = atan(gamecult_geometry_planetary_sinh(coordinate.y * CULTMATH_PI)); }
    else
    {
        float3 centered = gamecult_geometry_planetary_equal_earth_inverse(coordinate);
        longitude = atan2(centered.y, centered.x); latitude = asin(clamp(centered.z, -1.0, 1.0));
    }
    direction = gamecult_geometry_planetary_lon_lat_direction(gamecult_geometry_planetary_wrap_longitude(longitude + center_lon_lat_scale.x), latitude);
    return true;
}

struct GameCultGeometryPlanetaryFieldDefinition
{
    float radius;
    int seed;
    GameCultGeometryAdvancedErosionParameters erosion;
};

struct GameCultGeometryPlanetaryBaseFieldSample
{
...
struct GameCultGeometryPlanetarySurfaceSample
{
    float radial_displacement;
...
struct GameCultGeometryPlanetaryPageSample
{
    float4 height_gradient;
    float2 masks;
};

GameCultGeometryPlanetaryPageSample gamecult_geometry_planetary_page_lerp(
    GameCultGeometryPlanetaryPageSample a,
    GameCultGeometryPlanetaryPageSample b,
    GameCultGeometryPlanetaryPageSample c,
    GameCultGeometryPlanetaryPageSample d,
    float2 fraction)
{
    GameCultGeometryPlanetaryPageSample result;
...
GameCultGeometryPlanetaryPageSample gamecult_geometry_planetary_residual_sample(
    GameCultGeometryPlanetarySurfaceSample child,
    GameCultGeometryPlanetarySurfaceSample parent,
    bool has_parent)
{
    GameCultGeometryPlanetaryPageSample result;
...
float4 gamecult_geometry_planetary_summary_empty()
{
    return float4(3.402823466e+38, -3.402823466e+38, 0.0, 0.0);
}

float4 gamecult_geometry_planetary_summary_accumulate(float4 summary, float4 height_gradient, float unresolved_height)
{
...
float gamecult_geometry_planetary_pow4(float value) { float square = value * value; return square * square; }

GameCultGeometryPlanetarySurfaceSample gamecult_geometry_planetary_field_sample(
    ...
    float footprint_meters)
{
    ...
    GameCultGeometryAdvancedErosionParameters p = definition.erosion;
    GameCultGeometryErosionBandSelection band = gamecult_geometry_select_erosion_bands(
        p.scale * p.cell_scale, footprint_meters, p.octaves, p.lacunarity,
        p.strength * p.scale, p.gain, 2.0);
    float3 powers = float3(
        gamecult_geometry_planetary_pow4(abs(direction.x)),
        gamecult_geometry_planetary_pow4(abs(direction.y)),
        gamecult_geometry_planetary_pow4(abs(direction.z)));
    ...
    GameCultGeometryAdvancedErosionResult xy = gamecult_geometry_advanced_erosion_filter_banded(
    ...
    GameCultGeometryAdvancedErosionResult yz = gamecult_geometry_advanced_erosion_filter_banded(
    ...
    GameCultGeometryAdvancedErosionResult zx = gamecult_geometry_advanced_erosion_filter_banded(
    ...
    GameCultGeometryPlanetarySurfaceSample result;
    result.radial_displacement = base_sample.radial_displacement + erosion_height;
    result.tangent_gradient = gradient;
    result.surface_normal = gamecult_geometry_planetary_surface_normal(direction, gradient);

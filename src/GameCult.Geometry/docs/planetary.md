...
GameCult.Geometry owns the deterministic, renderer-independent planetary field. The same
...
- `IPlanetaryBaseField` supplies deterministic application-authored base
  evidence without making Geometry depend on an application.
...
HLSL lives in `Shaders/Planetary.hlsl`. Consumers include
`Shaders/GameCult.Geometry.hlsl`, which imports CultMath's numeric mirror before
...
`gamecult_geometry_planetary_face_direction` for procedural vertices and
`gamecult_geometry_planetary_radial_refinement_step` for bounded pixel refinement.
...
The `GameCult.Geometry.Unity` assembly in `org.gamecult.geometry` converts a canonical face patch into a
...
and strategic overlays do not enter GameCult.Geometry's canonical field.

`PlanetaryMapTileEncoding` writes the resulting evidence as a versioned
little-endian `CMPT` binary asset.
`packages/gamecult-geometry-ts/src/planetary-tile.ts` decodes that exact format
into browser-native values without defining another terrain generator.
Cross-runtime fixtures are generated by
`tools/GameCult.Geometry.PlanetaryTileFixture`.
